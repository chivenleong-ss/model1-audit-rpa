...
def _log_debug(self, msg, *a, **kw):
    self._push((msg if not a else msg % a))


def _log_critical(self, msg, *a, **kw):
    self.error(msg, *a, **kw)


if not hasattr(_Log, "debug"):
    _Log.debug = _log_debug
if not hasattr(_Log, "critical"):
    _Log.critical = _log_critical
...
def _pipeline(job_id, job_dir, v_path, d_path, i_path: str = ""):
    ...
    try:
        ...
        delivery = os.path.join(audit.result_dir, "交付版_效益审核表.xlsx")
