    ...
    def generate(self, df):
        ...
        try:
            ...
            for idx, row in df_calc.iterrows():
                ...
                debit = pd.to_numeric(row.get('借方本位币金额', 0), errors='coerce')
                credit = pd.to_numeric(row.get('贷方本位币金额', 0), errors='coerce')
                balance = pd.to_numeric(row.get('余额（本币）', 0), errors='coerce')

                cat, sub_cat, amt = None, None, 0.0

                # 1. 收入与税金映射
                if '主营业务收入' in gl_text:
                    cat, amt = '财务累计入账收入(不含增值税)', credit
                elif '应交税费\\增值税\\应交增值税\\销项税额' in gl_text:
                    cat, amt = '已价税分离的增值税金额(财务账面数据)', credit
                
                # 2. 核心成本映射
                elif '直接人工费' in gl_text:
                    cat, amt = '(一)人工费', debit
                elif '分包工程支出' in gl_text:
                    cat, amt = '(二)分包工程', debit
                
                elif '原材料' in gl_text:
                    if ('内部存款' in opp_gl_text or '内部往来' in opp_gl_text) and doc_no.startswith('DBD'):
                ...
                elif '合同履约成本\\工程施工成本\\直接材料费' in gl_text:
                    if ('内部存款' in opp_gl_text or '内部往来' in opp_gl_text) and doc_no.startswith('DBD'):
                        cat, sub_cat, amt = '(三)材料费', '材料调出', debit
                    elif opp_gl_text in ['', 'nan', 'None'] and doc_no.startswith('CZD'):
                        cat, sub_cat, amt = '(三)材料费', '废旧物资处置', debit
                    elif ('内部存款' in opp_gl_text or '内部往来' in opp_gl_text) and not doc_no.startswith('DBD'):
                        cat, sub_cat, amt = '(三)材料费', '其他', debit
                    else:
                        pass 

                elif '机械使用费' in gl_text:
                    cat, amt = '(四)机械租赁费', debit
                elif '其他直接费用' in gl_text:
                    cat = '(五)其他直接费'
                    sub_cat = gl_text.split('\\')[-1] if '\\' in gl_text else gl_text
                    amt = debit
                elif '间接费用' in gl_text:
                    cat = '(六)间接费'
                    sub_cat = gl_text.split('\\')[-1] if '\\' in gl_text else gl_text
                    amt = debit
                
                # 3. 投资收益
                elif '投资收益' in gl_text or '以摊余成本计量的金融资产终止确认收益' in gl_text:
                    cat, amt = '七、局投资收益（局投资项目选填）', credit
                
                # 4. 安全生产费智能细分
                elif '专项储备\\安全生产费\\发生数' in gl_text:
                    cat = '(七)安全费' 
                    if '内部存款' in opp_gl_text or '可用存款' in opp_gl_text:
                        sub_cat = '报销'
                        ...
                        if '合同履约成本' in opp_gl_text and '工程施工成本' in opp_gl_text:
                            sub_cat = opp_gl_text.split('\\')[-1] if '\\' in opp_gl_text else opp_gl_text.split('/')[-1]
                        else:
                            sub_cat = '其他'
                    amt = debit
                ...
                # ========================================================
                    ...
                    elif '人工成本' in gl_text:
                        sub_cat = '人工成本'
                        offset_cat, offset_subcat = '(一)人工费', '研发支出-人工成本'
                    elif '租赁及运行维护费' in gl_text:
                        sub_cat = '机械租赁'
                        offset_cat, offset_subcat = '(四)机械租赁费', '研发支出-机械租赁'
                    else:
                        if '折旧' in gl_text:
                            sub_cat = '折旧'
                        elif '内部存款' in opp_gl_text or '内部往来' in opp_gl_text:
                            sub_cat = opp_gl_text.split('\\')[-1] if '\\' in opp_gl_text else (opp_gl_text.split('/')[-1] if opp_gl_text else '内部结转')
                        else:
                            sub_cat = '其他'
            ...
            # 基础字段提炼 (消除 nan 干扰)
            vendor = df_valid.get('供应商名称', df_valid.get('供应商'))
            customer = df_valid.get('客户描述', df_valid.get('客户'))
            
            df_valid['客商名称'] = df_valid.get('客商名称', vendor.fillna(customer))
