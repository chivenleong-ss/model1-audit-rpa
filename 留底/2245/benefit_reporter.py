...
class BenefitReporter:
    ...
    def _style_row(self, ws, r, ncol=22):
    ...
    def _write_row(self, ws, r, d_val, c_val, m_val, extended):
        self._style_row(ws, r)
    ...
    def _insert_rows(self, ws, anchor, action, rows_data, extended):
        for i, rd in enumerate(rows_data):
            ...
            ws.insert_rows(ins)
            self._write_row(ws, ins, rd.get('d'), rd.get('c'), rd.get('p', 0), extended)
    ...
    def _build_lookups(self, df4):
        # 客商名称+合同 → 供应商编码
        vendor_lkp = {}
        sub = df4[df4['供应商名称'].notna() & df4['供应商'].notna()]
        for _, row in sub.drop_duplicates(['供应商名称', '合同']).iterrows():
            ven  = self._norm(str(row['供应商名称']))
        ...
        ap = df4[df4['总账科目长文本'].fillna('').str.contains('应付账款') &
