...
class IntermediateTableBuilder:
    ...
    def generate(self, df):
        ...
        try:
            ...
            # 新规则：对方科目为空时，优先按“中台单据号”反查三级科目
            if '中台单据号' in cost_df.columns:
                valid_doc_df = cost_df[cost_df['中台单据号'].notna() & (cost_df['中台单据号'].astype(str).str.strip() != '')].copy()
            ...
            for idx, row in df_calc.iterrows():
                ...
                # 2. 核心成本映射
                    ...
                    elif ('可用存款' in opp_gl_text_path or '内部存款' in opp_gl_text_path or '其他应收款' in opp_gl_text_path) and doc_no.upper().startswith('CFK'):
                ...
                # 4. 安全生产费智能细分（仅分类，不再同步抵消）
                    ...
                    if '内部存款' in opp_gl_text_path or '可用存款' in opp_gl_text_path:
                        # 对方科目为内部存款/可用存款等 → 报销类，不做跨类抵消
                        sub_cat = '安全生产费-报销'
                    elif opp_gl_text.strip() in ['', 'nan', 'None']:
                        # 对方科目为空 → 优先通过“中台单据号”反查；兜底再用“文本”
                        ...
                        if '合同履约成本' in opp_gl_text_path and '工程施工成本' in opp_gl_text_path:
                            opp_leaf = opp_gl_text_path.split('\\')[-1] if '\\' in opp_gl_text_path else opp_gl_text_path.split('/')[-1]
                            # 尝试从对方科目末级匹配
                ...
                if doc_no.upper().startswith('ZGD'):
                    sub_cat = 'ZGD'
                    df_calc.at[idx, '客商名称'] = 'ZGD'  # 强行统一客商名，确保透视时合并
                    df_calc.at[idx, '合同编码'] = ''     # 清空合同号，避免被拆散成多行
                    amt = debit                        # 强制取借方本位币金额合计

                # 回填计算结果  <--- (大约在原代码的 217 行附近)
            ...
            # 缺失客商时的兜底规则：缺失客商名称，则按中台单据号的前三位字母代替
            def _fallback_vendor_name(row):
                ...
                # 2. 如果客商名称缺失，尝试去取“中台单据号”的前三位
                doc_no = str(row.get('中台单据号', '') or '').strip()
                if doc_no and doc_no.lower() not in ('nan', 'none'):
                    return doc_no[:3].upper()  # 截取前3位并转为大写（如 FGD, CFK）
                
                # 3. 极端情况：连单据号都没有，才返回空白
                return ''
