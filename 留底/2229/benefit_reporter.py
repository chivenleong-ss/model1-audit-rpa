...
class BenefitReporter:
    ...
    def _style_row(self, ws, r, ncol=22, fill_ref_row: int | None = None):
        thin = Side(border_style="thin", color="000000")
        border = Border(left=thin, right=thin, top=thin, bottom=thin)
        font   = Font(name="宋体", size=10)
        if fill_ref_row is None:
            fill_ref_row = r - 1 if r > 1 else None
        for c in range(1, ncol + 1):
            ...
            # 复制参考行的填充色（默认上一行）
            if fill_ref_row and 1 <= fill_ref_row <= ws.max_row:
                self._copy_fill(ws.cell(fill_ref_row, c), cell)
    ...
    def _write_row(self, ws, r, d_val, c_val, m_val, extended, fill_ref_row: int | None = None):
        self._style_row(ws, r, fill_ref_row=fill_ref_row)
    ...
    def _fix_rnd_subtotal(self, ws):
        """
        修复“八、研发费用”小计行公式范围，避免插行后仍引用模板旧行号。
        """
        rnd_row = self._find_row(ws, '八、研发费用')
        total_row = self._find_row(ws, '九、成本及费用合计') or self._find_row(ws, '八、成本及费用合计')
        if not rnd_row or not total_row or total_row <= rnd_row:
            return
        start_row = rnd_row + 1
        end_row = max(start_row, total_row - 1)
        for col in range(1, ws.max_column + 1):
            cell = ws.cell(rnd_row, col)
            if cell.value and isinstance(cell.value, str) and 'SUM(' in cell.value.upper():
                cl = get_column_letter(col)
                cell.value = f"=SUM({cl}{start_row}:{cl}{end_row})"
        self.log.info("   🧮 研发费用小计 SUM 范围 行%d→%d", start_row, end_row)
    ...
    def _fix_aggregate_rows(self, ws):
        ...
        # ── 八、成本及费用合计 ──────────────────────────────
        total_row = self._find_row(ws, '八、成本及费用合计') or self._find_row(ws, '九、成本及费用合计')
    ...
    def _agg_labor(self, df):
        """人工费：FGD→'FGD'汇总，CFK→'CFK+客商名称'分组，其余按客商+合同"""
        df = df.copy()
        ...
        for ven, grp in df[df['_pfx'] == 'CFK'].groupby(df['客商名称'].fillna('')):
            rows.append({"d": f"CFK+{ven}", "c": None, "p": round(grp['最终发生额'].sum(), 2)})
        others = df[~df['_pfx'].isin(['FGD', 'CFK'])]
        for (ven, con), grp in others.groupby(
                [others['客商名称'].fillna(''), others['合同编码'].fillna('')], sort=False):
    ...
    def _insert_rows(self, ws, anchor, action, rows_data, extended):
        for i, rd in enumerate(rows_data):
            ins = anchor + i if action == 'above' else anchor + 1 + i
            ws.insert_rows(ins)
            # above: 参考上一行；below: 参考下一行（避免继承到节标题底色）
            if action == 'below' and (ins + 1) <= ws.max_row:
                fill_ref = ins + 1
            else:
                fill_ref = ins - 1 if ins > 1 else None
            self._write_row(
                ws, ins, rd.get('d'), rd.get('c'), rd.get('p', 0), extended,
                fill_ref_row=fill_ref
            )
    ...
    def execute_fill(self):
        ...
        try:
            ...
            for cat, mode, anchor_kw, action, ext in plan:
                ...
                self._insert_rows(ws, anchor, action, rows_data, ext)
            ...
            self._fix_rnd_subtotal(ws)       # ③ 研发费用小计 SUM 范围修正
